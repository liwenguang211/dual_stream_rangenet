...
| `run_loeo_9fold.py` | Production training script using the canonical 16-channel dataset, full DS-RangeNet with CBAM/IGCA, validation-only checkpoint selection, and one-time KNN-refined test evaluation. |
...
# 2. From the repository root, run 9-fold LOEO training (GPU + UBPC-9 required)
python loeo_supporting_materials/run_loeo_9fold.py \
  --config loeo_supporting_materials/ubpc9_splits_v3.json \
  --output-dir loeo_logs \
  --raw results/raw/loeo_folds_measured.csv

# 3. Verify the generated summary matches the paper tables
python3 verify_loeo_audit.py
```

The script writes one validation-selected checkpoint per fold and records the
measured fold-level and per-class test IoUs in
`results/raw/loeo_folds_measured.csv`. These measurements are kept separate
from the paper's existing result file until the full run has completed and the
values have been independently checked.
